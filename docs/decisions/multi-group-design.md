...
- **Changing it later:** with nobody set, the Stats tab's "Pick an active user…" hint is a button that opens the same picker. Changing an already-chosen person was deferred at the time; [#99](https://github.com/sharneng/spliit2go/issues/99) added it on the Balances tab (next section). The group-settings checkbox originally proposed in #83 was dropped: the active user is device-only, while group settings saves to the server.

## "You" on the Balances tab (2026-09-24, issue #99)

Kenneth asked for spliit-ios's "You" section on Balances (`BalancesView.youSection` @ `80b2e98`; spliit-web has none). Merged in [#101](https://github.com/sharneng/spliit2go/pull/101). It is also where the active user is changed: until then, once someone was picked there was no way to change it.
