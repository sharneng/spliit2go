...
- **A step after the save itself fails:** the save stands. If an expense is saved but its split can't be remembered as the default, the form closes as usual with a snack bar note (Details, since it's unexpected), rather than offering a retry that would add the expense again. The split can be saved as the default with the next expense (Kenneth and Ezra, #119 review).
